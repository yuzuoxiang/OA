            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            rndFileName = Tools.Usual.Utils.ConverUrl(Tools.StrClass.StringProcess.CutString(title, 50)).TrimEnd('-') + "-" + rndFileName;
                        }
                        ...
                        if (status == "success")
                        {
                            ...
                            {
                                // 限制图片大小
                                ImageHelper.SetImgSize(dirPath + newFileName, setting.MaxWidth, setting.MaxHeight);
                                ...
                                {
                                    ImageHelper.CreateSmallImage(dirPath + newFileName, dirPath + newSmallFileName, setting.Width, setting.Height);
                                }
                                //生成中图
                                if (setting.CreateMiddlePic)
                                {
                                    ImageHelper.CreateSmallImage(dirPath + newFileName, dirPath + newMiddleFileName, setting.MiddleWidth, setting.MiddleHeight);
                                }
                                //生成大图
                                if (setting.CreateBigPic)
                                {
                                    ImageHelper.CreateSmallImage(dirPath + newFileName, dirPath + newBigFileName, setting.BigWidth, setting.BigHeight);
                                }
                                //打水印
                                if (setting.AddWaterMark)
                                {
                                    ...
                                    ImageHelper.AddWaterMark(dirPath + newFileName, dirPath + newFileName, waterMarkImgOrTxt, wmtype, WaterMarkPosition.Right_Bottom, setting.Transparency);
                                    ImageHelper.AddWaterMark(dirPath + newBigFileName, dirPath + newBigFileName, waterMarkImgOrTxt, wmtype, WaterMarkPosition.Right_Bottom, setting.Transparency);
                                }
                            }
                            #endregion
                        }
