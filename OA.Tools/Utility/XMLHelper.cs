...
namespace Tools.Utility
{
    ...
    public class XMLHelper
    {
        ...
        public static XmlNodeList GetXmlNodeListByXpath(string xmlFileName, string xpath)
        {
            ...
            {
                Logs.Log.Error("Xml匹配XPath表达式的节点列表错误", typeof(XMLHelper).ToString(), ex);
                return null;
                //throw ex; //这里可以定义你自己的异常处理
            }
        }
        ...
        public static XmlNodeList GetXmlNodeListByXpathFromStr(string xml, string xpath)
        {
            ...
            {
                Logs.Log.Error("Xml选择匹配XPath表达式的节点列表XmlNodeList错误", typeof(XMLHelper).ToString(), ex);
                return null;
                //throw ex; //这里可以定义你自己的异常处理
            }
        }
        ...
        public static int GetXmlChildCnt(string xmlFileName)
        {
            ...
            {
                Logs.Log.Error("Xml获取一级节点子节点数量错误", typeof(XMLHelper).ToString(), ex);
                return 0;
                //throw ex; //这里可以定义你自己的异常处理
            }
        }
