...
namespace Tools.Tool
{
    ...
    public class PathHandle
    {
        ...
        public static string GetWebPath()
        {
            string path = HttpContext.Current.Request.ApplicationPath;
            //如果不是根目录就加上"/" 根目录自己会加"/"
            string thisPath = path != "/" ? path + "/" : path;
            return thisPath;
        }
        ...
        public static string GetFilePath(string localPath)
        {
            if (localPath == null)
                return string.Empty;
            else if (System.Text.RegularExpressions.Regex.IsMatch(localPath, @"([A-Za-z]):\\([\S]*)"))
                return localPath;
            else
            {
        ...
        public static string GetVirtualPath(string parmStr)
        {
            ...
            return string.Concat(vUrl, "?", parmStr.Replace("?", ""));
        }
    }
}
